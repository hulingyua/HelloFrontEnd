...
1. 创建目录 `c:\data\db\ `
   ...
   - e.g. mongod --dbpath D:\mongo\data\db\
...
语法: use databaseName // 切换 || 创建数据库
```

##### 3.  查看当前所处的数据库

```javascript
语法: db // 如果没有指定具体的数据库就会返回一个测试的: test
```
...
语法: db.[collectionName].update({update criteria},{$set:{fields to update}})
// 示例
...
```javascript
// 删除指定集合中的数据
语法: db.[collectionName].remove({deletion condition})
```
